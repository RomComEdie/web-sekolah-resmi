...
async function initDatabase() {
  ...
  try {
    ...
    isDbConnected = false;
    console.log(`ℹ️ [Database Status] MySQL XAMPP connection: ${err.message || 'Offline'}. Using in-memory store mode.`);
  }
}
...
// Delete PPDB Record
app.delete(['/api/ppdb/:id', '/backend/api/ppdb.php/:id'], (req, res) => {
  const { id } = req.params;
...
// Update PPDB Status
app.patch(['/api/ppdb/:id', '/backend/api/ppdb.php/:id'], (req, res) => {
  const { id } = req.params;
...
// Express PPDB Status Check Endpoint
app.get(['/api/ppdb/check-status', '/backend/api/ppdb_status.php'], (req, res) => {
...
app.get(['/api/announcements', '/backend/api/announcements.php'], (req, res) => {
  res.json({
    status: 'success',
    code: 200,
...
app.post(['/api/announcements', '/backend/api/announcements.php'], (req, res) => {
  const sanitized = sanitizeInput(req.body);
  const newAnn = {
    id: `ANN-${Date.now()}`,
    title: sanitized.title || 'Pengumuman Baru',
    category: sanitized.category || 'Pengumuman',
    date: sanitized.date || new Date().toISOString().split('T')[0],
    summary: sanitized.summary || sanitized.content?.substring(0, 120) || '',
    content: sanitized.content || '',
    author: sanitized.author || 'Super Admin Website',
...
app.put(['/api/announcements/:id', '/backend/api/announcements.php/:id'], (req, res) => {
  const { id } = req.params;
  const sanitized = sanitizeInput(req.body);
...
app.delete(['/api/announcements/:id', '/backend/api/announcements.php/:id'], (req, res) => {
  const { id } = req.params;
...
// Express PPDB API Endpoint (Fallback & Native Node backend route)
app.get(['/api/ppdb', '/backend/api/ppdb.php'], (req, res) => {
...
app.post(['/api/ppdb', '/backend/api/ppdb.php'], (req, res) => {
  ...
  const existing = registrationsStore.find(
    (r) => r.nikNisn === String(input.nikNisn).trim()
  );
  if (existing) {
...
app.post(['/api/pesan', '/backend/api/pesan.php'], submitLimiter, (req, res) => {
  const body = req.body || {};
...
app.get(['/api/pesan', '/backend/api/pesan.php'], (req, res) => {
  res.json({
    status: 'success',
    code: 200,
    data: messagesStore
