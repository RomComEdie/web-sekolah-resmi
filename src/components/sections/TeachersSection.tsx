...
export const TeachersSection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('smk_teachers_updated', handleUpdate);
    window.addEventListener('storage', handleUpdate);
    return () => {
      window.removeEventListener('smk_teachers_updated', handleUpdate);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 mb-10">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
            ...
            {/* Kelola Profil Guru CMS Button */}
            <button
              onClick={() => {
                window.dispatchEvent(new CustomEvent('open_admin_cms', { detail: { subTab: 'GURU_CMS' } }));
              }}
              className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-[#1b3828] text-[#c5a059] font-black text-xs hover:bg-[#2d5a3f] transition-all flex items-center justify-center gap-2 shadow-sm border border-[#c5a059]/40 cursor-pointer shrink-0"
              title="Kelola & Edit Profil Guru di Admin Portal"
            >
              <Edit size={15} />
              <span>Kelola & Edit Profil Guru</span>
            </button>
          </div>
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl border border-slate-200 space-y-0 relative animate-scaleUp">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="pt-2 flex flex-col sm:flex-row gap-2">
                <button
                  onClick={() => {
                    const teacherId = selectedTeacherModal.id;
                    setSelectedTeacherModal(null);
                    window.dispatchEvent(new CustomEvent('open_admin_cms', { detail: { subTab: 'GURU_CMS', teacherId } }));
                  }}
                  className="flex-1 py-2.5 rounded-xl bg-amber-50 text-amber-900 border border-amber-300 font-extrabold text-xs hover:bg-amber-100 transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                >
                  <Edit size={15} />
                  <span>Edit Profil Guru Ini</span>
                </button>

                <button
                  onClick={() => setSelectedTeacherModal(null)}
                  className="py-2.5 px-5 rounded-xl bg-[#1b3828] text-white font-bold text-xs hover:bg-[#2d5a3f] transition-colors cursor-pointer"
                >
                  Tutup Profil
                </button>
              </div>
