...
export const PpdbSection: React.FC<PpdbSectionProps> = ({ initialMajorChoice = 'RPL' }) => {
  ...
  const [birthPlaceSelect, setBirthPlaceSelect] = useState<string>('Banjarmasin');
  const [customBirthPlace, setCustomBirthPlace] = useState<string>('');
  const [birthDay, setBirthDay] = useState<string>('15');
  const [birthMonth, setBirthMonth] = useState<string>('05');
  const [birthYear, setBirthYear] = useState<string>('2010');
  ...
  // Derived birth place & birth date values
  const birthPlace = birthPlaceSelect === 'Lainnya' ? customBirthPlace : birthPlaceSelect;
  const birthDate = (birthYear && birthMonth && birthDay) ? `${birthYear}-${birthMonth}-${birthDay}` : '';

  // Options for scrollable city & date pickers
  ...
  const YEARS = Array.from({ length: 22 }, (_, i) => String(2016 - i));

  // Helper to format Indonesian Date (e.g. 2010-08-15 -> 15 Agustus 2010)
  ...
  // Helper to validate Indonesian Phone Numbers (Must start with 08 or +62)
  const isValidIndonesianPhone = (phone: string) => {
    ...
    if (cleaned.startsWith('+62') && (cleaned.length < 11 || cleaned.length > 16)) {
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!birthPlace.trim()) {
      setErrorMessage('Kota / Tempat Lahir Wajib diisi!');
      return;
    }
    if (!birthDate) {
      setErrorMessage('Tanggal Lahir Wajib dipilih!');
      return;
    }
    if (!address.trim()) {
    ...
    if (!isValidIndonesianPhone(phoneWhatsapp)) {
      setErrorMessage('Nomor WhatsApp / HP Aktif Wajib diawali dengan 08 atau +62 (Contoh: 081234567890 atau +6281234567890) dengan minimal 10 digit!');
      return;
    }
    if (parentPhone.trim() && !isValidIndonesianPhone(parentPhone)) {
      setErrorMessage('Nomor HP Orang Tua / Wali Wajib diawali dengan 08 atau +62 (Contoh: 081234567890 atau +6281234567890)!');
      return;
    }
    ...
    const computedBirthPlaceDate = `${birthPlace.trim()}, ${formatIndonesianDate(birthDate)}`;

    const payload = {
      programType: 'MPLS / PPDB Siswa Baru' as const,
      fullName: fullName.trim(),
      nikNisn: nikNisn.trim(),
      birthPlaceDate: computedBirthPlaceDate,
      gender,
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    {/* Tempat & Tanggal Lahir (Scrollable Select Pickers) */}
                    <div className="sm:col-span-2 bg-emerald-50/20 p-4 rounded-2xl border border-slate-200 space-y-3">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {/* Kota / Tempat Lahir (Scrollable City Select) */}
                        <div>
                          <label className="block text-xs font-bold text-slate-700 mb-1 flex items-center gap-1">
                            <MapPin size={13} className="text-[#c5a059]" />
                            <span>Kota / Tempat Lahir</span> <span className="text-red-500">*</span>
                          </label>
                          <select 
                            value={birthPlaceSelect}
                            onChange={(e) => setBirthPlaceSelect(e.target.value)}
                            className="w-full px-4 py-3 rounded-xl bg-white border border-slate-300 text-sm focus:outline-none focus:border-[#2d5a3f] font-medium cursor-pointer"
                          >
                          ...
                          {/* Manual Input if 'Lainnya' selected */}
                          {birthPlaceSelect === 'Lainnya' && (
                            <input 
                              type="text"
                              required
                              ...
                              className="mt-2 w-full px-4 py-2.5 rounded-xl bg-white border border-slate-300 text-sm focus:outline-none focus:border-[#2d5a3f]"
                            />
                          )}
                        </div>

                        {/* Tanggal Lahir (Scrollable Day, Month, Year Selects) */}
                        <div className="w-full max-w-full overflow-hidden">
                          <label className="block text-xs font-bold text-slate-700 mb-1 flex items-center gap-1">
                            <Calendar size={13} className="text-[#c5a059]" />
                            <span>Tanggal Lahir</span> <span className="text-red-500">*</span>
                          </label>
                          <div className="grid grid-cols-3 gap-1 sm:gap-2 w-full max-w-full">
                            ...
                            <select
                              ...
                              className="w-full max-w-full px-1.5 sm:px-2 py-2.5 sm:py-3 rounded-xl bg-white border border-slate-300 text-[11px] sm:text-xs font-bold text-slate-800 focus:outline-none focus:border-[#2d5a3f] cursor-pointer truncate"
                              title="Pilih Tanggal (Hari)"
                            >
                              {DAYS.map((d) => (
                      ...
                      {/* Live Date Preview */}
                      {(birthPlace.trim() || birthDate) && (
                        <div className="text-xs text-[#2d5a3f] font-semibold bg-[#e8f0eb] border border-[#2d5a3f]/30 px-3 py-2 rounded-xl flex items-center gap-2">
                          <Calendar size={14} className="text-[#c5a059] shrink-0" />
                          <span>
                            Data Lahir Tersimpan: <strong>{birthPlace.trim() || '...'}{birthDate ? `, ${formatIndonesianDate(birthDate)}` : ''}</strong>
                          </span>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input 
                        ...
                        className={`w-full px-4 py-3 rounded-xl text-sm focus:outline-none font-medium transition-colors ${
                          phoneWhatsapp.trim().length > 0 && !isValidIndonesianPhone(phoneWhatsapp)
                            ? 'bg-rose-50/60 border-2 border-rose-400 focus:border-rose-500 text-rose-900'
                            : phoneWhatsapp.trim().length > 0 && isValidIndonesianPhone(phoneWhatsapp)
                            ? 'bg-emerald-50/40 border-2 border-emerald-500 focus:border-emerald-600'
                            : 'bg-[#FAFBF9] border border-slate-300 focus:border-[#2d5a3f] focus:ring-1 focus:ring-[#2d5a3f]'
                        }`}
                      />
                      {/* Real-time Phone Error Display */}
                      {phoneWhatsapp.trim().length > 0 && !isValidIndonesianPhone(phoneWhatsapp) && (
                        <div className="mt-1.5 text-[11px] font-semibold text-rose-600 flex items-center gap-1">
                          <AlertCircle size={13} className="shrink-0" />
                          <span>
                            {!phoneWhatsapp.startsWith('08') && !phoneWhatsapp.startsWith('+62')
                              ? 'Nomor HP harus diawali dengan 08 atau +62 (Contoh: 081234567890)'
                              : 'Nomor HP tidak valid (minimal 10 digit)'}
                          </span>
                        </div>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <input 
                        ...
                        className={`w-full px-4 py-3 rounded-xl text-sm focus:outline-none font-medium transition-colors ${
                          parentPhone.trim().length > 0 && !isValidIndonesianPhone(parentPhone)
                            ? 'bg-rose-50/60 border-2 border-rose-400 focus:border-rose-500 text-rose-900'
                            : parentPhone.trim().length > 0 && isValidIndonesianPhone(parentPhone)
                            ? 'bg-emerald-50/40 border-2 border-emerald-500'
                            : 'bg-[#FAFBF9] border border-slate-300 focus:border-[#2d5a3f]'
                        }`}
                      />
                      {/* Real-time Parent Phone Error Display */}
                      {parentPhone.trim().length > 0 && !isValidIndonesianPhone(parentPhone) && (
                        <div className="mt-1.5 text-[11px] font-semibold text-rose-600 flex items-center gap-1">
                          <AlertCircle size={13} className="shrink-0" />
                          <span>
                            {!parentPhone.startsWith('08') && !parentPhone.startsWith('+62')
                              ? 'Nomor HP orang tua harus diawali 08 atau +62'
                              : 'Nomor HP tidak valid (minimal 10 digit)'}
                          </span>
                        </div>
                      )}
