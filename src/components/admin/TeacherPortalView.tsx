    ...
    <div className="min-h-screen bg-[#f4f6f4] text-slate-800 flex flex-col font-sans selection:bg-[#c5a059] selection:text-[#1b3828]">
      ...
      <div className="flex flex-1 overflow-hidden relative">
          ...
          {isMobileSidebarOpen && (
            <>
              ...
              <motion.aside
                ...
                className="fixed top-0 left-0 bottom-0 w-72 bg-[#1b3828] text-white z-50 flex flex-col border-r border-[#2d5a3f] shadow-2xl md:hidden overflow-y-auto"
              >
                {/* Mobile Drawer Header */}
                <div className="p-4 border-b border-[#2d5a3f] flex items-center justify-between bg-[#142b1f]">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-2xl bg-white p-1 flex items-center justify-center font-black border-2 border-[#c5a059] shadow-md overflow-hidden shrink-0">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-[#c5a059] font-bold block">Portal Guru & Wali Kelas</span>
                    </div>
                  </div>
                  <button
                    onClick={() => setIsMobileSidebarOpen(false)}
                ...
                <div className="p-4 border-t border-[#2d5a3f] space-y-2 bg-[#142b1f]">
                  {onBackToAdmin && (
                    <button
                      onClick={() => {
                        onBackToAdmin();
                      ...
                      className="w-full py-2.5 px-3 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 text-amber-200 border border-amber-400/30 text-xs font-bold flex items-center justify-center gap-2 transition-all min-h-[44px]"
                    >
                      <Building2 size={16} />
        ...
        <motion.aside
          ...
          className="hidden md:flex bg-[#1b3828] text-white border-r border-[#2d5a3f] flex-col shrink-0 z-20 shadow-xl transition-all duration-300"
        >
          {/* Sidebar Header Brand */}
          <div className="p-5 border-b border-[#2d5a3f] flex items-center justify-between">
            <div className="flex items-center gap-3 overflow-hidden">
              <div className="w-12 h-12 rounded-2xl bg-white p-1 border-2 border-[#c5a059] flex items-center justify-center font-black shadow-md shrink-0 overflow-hidden">
                <img src={SCHOOL_INFO.logoUrl} alt="Logo SMK" className="w-full h-full object-contain" />
              </div>
              {isSidebarOpen && (
                <motion.div
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
          ...
          {/* TOP NAVBAR */}
          <header className="bg-white border-b border-slate-200 px-4 md:px-6 py-3.5 flex items-center justify-between gap-3 sticky top-0 z-10 shadow-2xs">
            {/* Mobile Sidebar Hamburger Toggle */}
            <button
              onClick={() => setIsMobileSidebarOpen(true)}
              className="p-2.5 rounded-2xl bg-[#1b3828] text-white hover:bg-[#2d5a3f] transition-colors md:hidden shrink-0 border border-[#c5a059]/40"
              title="Buka Menu Portal Guru"
            >
          ...
          {/* PAGE BODY CONTENT */}
          <main className="p-4 sm:p-6 md:p-8 space-y-5 sm:space-y-6 max-w-7xl mx-auto w-full">

            {/* BREADCRUMB & HEADER ACTION BAR */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
              <div>
                <h2 className="text-lg sm:text-2xl font-black text-[#1b3828] tracking-tight leading-tight">
                ...
                <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 text-[11px] sm:text-xs font-semibold text-slate-500 mt-1">
                  <span>Home</span>
                  <span>/</span>
                  <span className="text-[#1b3828] font-bold">{activeMenu}</span>
                  <span>/</span>
                  <span className="text-[#c5a059] font-extrabold uppercase truncate max-w-[180px] sm:max-w-none">Wali Kelas {currentTeacher.name}</span>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full sm:w-auto px-5 py-2.5 sm:px-6 sm:py-3 rounded-2xl bg-gradient-to-r from-[#1b3828] via-[#2d5a3f] to-[#1b3828] text-[#c5a059] font-black text-xs uppercase tracking-wider shadow-lg shadow-[#1b3828]/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 border border-[#c5a059]/40 min-h-[44px]"
              >
