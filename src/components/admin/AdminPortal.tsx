...
import { TEACHERS_DATA, EXTRACURRICULARS, SCHOOL_INFO } from '../../data/schoolData';
import { 
  adminLogin, 
...
export const AdminPortal: React.FC<AdminPortalProps> = ({ isOpen, onClose }) => {
  ...
  const [isWebsiteSettingsSavedState, setIsWebsiteSettingsSavedState] = useState(false);
  ...
  const handleSaveWebsiteSettings = (e: React.FormEvent) => {
    ...
    setIsWebsiteSettingsSavedState(true);
    setTimeout(() => {
      setShowSavedToast(false);
      setIsWebsiteSettingsSavedState(false);
    }, 3500);
  };
        ...
        <div className="min-h-screen flex flex-col justify-between bg-[#e4ece7] text-slate-800 relative overflow-y-auto p-4 sm:p-6 font-sans">
          ...
          <header className="max-w-4xl w-full mx-auto flex items-center justify-between py-2 relative z-10">
            <div className="flex items-center gap-2.5">
              <div className="w-12 h-12 rounded-2xl bg-white p-1 flex items-center justify-center font-black shadow-[4px_4px_8px_#c2cebe,-4px_-4px_8px_#ffffff] border-2 border-[#c5a059] overflow-hidden shrink-0">
          ...
          <main className="flex-1 max-w-lg w-full mx-auto flex flex-col items-center justify-center relative z-10 my-auto py-6">
            ...
            >
              ...
              <div className="relative">
                <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-3xl bg-white p-2 flex items-center justify-center shadow-[inset_2px_2px_4px_rgba(0,0,0,0.1),4px_4px_12px_rgba(0,0,0,0.2)] border-3 border-[#c5a059] overflow-hidden">
                  <img src={SCHOOL_INFO.logoUrl} alt="Logo SMK Bhinneka Nusantara" className="w-full h-full object-contain" />
                </div>
                <div className="absolute -bottom-1 -right-1 w-8 h-8 rounded-full bg-[#c5a059] text-[#1b3828] flex items-center justify-center font-black shadow-md text-xs border-2 border-white">
                  <ShieldCheck size={18} />
                </div>
              </div>
                  ...
                  <form onSubmit={handleSaveWebsiteSettings} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="bg-emerald-900 text-white p-6 rounded-3xl border border-emerald-800 space-y-4 shadow-lg">
                        ...
                        <button
                          type="submit"
                          className={`w-full py-3.5 px-4 rounded-xl font-black text-xs transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer ${
                            isWebsiteSettingsSavedState
                              ? 'bg-emerald-400 text-slate-950 shadow-emerald-500/50 scale-[1.02] border-2 border-emerald-200'
                              : 'bg-[#c5a059] text-[#1b3828] hover:bg-white'
                          }`}
                        >
                          {isWebsiteSettingsSavedState ? (
                            <>
                              <CheckCircle2 size={18} className="text-slate-950 animate-bounce shrink-0" />
                              <span>BERHASIL DIUBAH & DIPUBLIKASIKAN!</span>
                            </>
                          ) : (
                            <>
                              <CheckCircle size={18} />
                        ...
                        {isWebsiteSettingsSavedState && (
                          <motion.div
                            ...
                            animate={{ opacity: 1, y: 0 }}
                            className="p-3 bg-emerald-400/20 border border-emerald-300/40 rounded-xl text-emerald-100 text-xs font-bold flex items-center gap-2"
                          >
                            <CheckCircle2 size={16} className="text-emerald-300 shrink-0" />
                            <span>Perubahan status PPDB & konfigurasi website berhasil disimpan dan dipublikasikan secara langsung!</span>
                          </motion.div>
                        )}
                      </div>
